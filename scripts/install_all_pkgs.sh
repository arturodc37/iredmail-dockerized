...
PKGS_PHP_FPM="php8.1-fpm php8.1-cli"

PKGS_ROUNDCUBE="php8.1-bz2 php8.1-curl php8.1-gd php8.1-imap php8.1-intl php8.1-ldap php8.1-mbstring php8.1-mysql php8.1-pgsql php8.1-pspell php8.1-xml php8.1-zip php8.1-mcrypt mariadb-client aspell"
...
# Install iRedAPD.
wget -c -q https://github.com/iredmail/iRedAPD/archive/6.0.tar.gz && \
tar xzf 6.0.tar.gz -C /opt && \
rm -f 6.0.tar.gz && \
ln -s /opt/iRedAPD-6.0 /opt/iredapd && \
chown -R iredapd:iredapd /opt/iRedAPD-6.0 && \
chmod -R 0500 /opt/iRedAPD-6.0 && \
...
# Install Roundcube.
wget -c -q https://github.com/roundcube/roundcubemail/releases/download/1.6.12/roundcubemail-1.6.12-complete.tar.gz && \
tar zxf roundcubemail-1.6.12-complete.tar.gz -C /opt/www && \
rm -f roundcubemail-1.6.12-complete.tar.gz && \
ln -s /opt/www/roundcubemail-1.6.12 /opt/www/roundcubemail && \
chown -R root:root /opt/www/roundcubemail-1.6.12 && \
chmod -R 0755 /opt/www/roundcubemail-1.6.12 && \
cd /opt/www/roundcubemail-1.6.12 && \
